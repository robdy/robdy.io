...
If we apply the policy to root folder, it'd be applied to all folder which are set to inherit from parent.

<Note>

Remember about the rule regarding personal tags. Root folder is default folder. Therefore only admin-defined retention policies can be applied.

There's one specific thing about root folder. From what I checked, personal archive tags cannot be applied to root folder.

### CHECK WHAT TAGS CAN BE APPLIED
...
That's all. AutoArchive is now enabled. Every 14 days (or whatever period we set) we'll be prompted to confirm the cleanup operation:
...
*Sweep rules* are another way to clean up emails. This mechanism is a bit different from the others described above.
...
## Cleaning up the folder using inbox rules
