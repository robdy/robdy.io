...
The first argument in the array specifies the file (or files if we use wildcard) to be given to Mocha. The next two overrides the default timeout value. They might be treated as optional. When we run a single set of tests, we usually won't exceed the default timeout (which is 2000ms).

The last argument tells Mocha to ignore its config file.
