...
The solution described above is not 100% the same as what Microsoft is providing, as it only applies to domain-joined Windows devices.
...
We finally got an option to deploy custom Teams backgrounds to the entire organization. It is very handy idea, which might help people to celebrate and increase visibility of some special events within the organization.

## License is required

The description says that Advanced Communications license will be required at some point. We already got number of announcements about that license. Microsoft changed their strategy few times, most likely getting into account negative feedback from the community.

However, we need to consider the option that Advanced Communications license will be enforced at some point. Many organizations will decide to not buy this license. In that case, the organizations will look for an alternative options.

In this article you'll learn multiple options to deploy Teams backgrounds to your users. None of the options described below require Advanced Communications license. 

## Table of Contents

```toc
exclude: [Table of Contents, License is required]
```
...
2. For each image check if it already exists
3. If the image is not yet downloaded, pull it from the link
...
* no mobile devices - the solutions described above only applies to Windows machines
