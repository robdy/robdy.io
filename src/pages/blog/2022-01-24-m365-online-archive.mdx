...
If you use or want to use Online Archive in Microsoft 365, this article is for you.
...
Archiving will start as soon as the archive mailbox is enabled. It requires proper archive policy. Administrators don't need to define such a policy. There's a default one, which archives all items older than 2 years.

## How do I check my archiving settings?

We can check archiving settings using the **Assign policy** button in Outlook. In Outlook on the web, we'll see **Assign policy** entry in the context menu. The menu is visible when we right-click the folder.
...
We'll notice that default option is to use parent folder policy. For folders like *Inbox*, *Sent Items* and others, the parent folder will be the root folder.
...
![](../../img/20220114-085209-Pa3zj3OD3P.png "Root folder in Outlook on the web and Outlook ")
