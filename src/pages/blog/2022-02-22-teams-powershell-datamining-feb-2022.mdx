...
The cmdlet provides a way to test the unassigned number treatment configured in our tenant. We specify the number using `-PhoneNumber` and it checks against all the unassigned number treatment rules. Basically, we can check if our regular expression is correct.
...
The fefault value for the parameter is *FullInformation*. We can guess we will be able to limit what information is available in report mode for the meeting.
