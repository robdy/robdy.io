...
We got 1 new cmdlet to test unassigned number treatment. 6 new cmdlets for Shifts appeared in preview.

Like previously, there was a number of cmdlets being removed in stable version and added back in the preview release. This article lists only the new ones.
...
The cmdlet provides a way to test unassigned number treatment configured in our tenant. We specify the number using `-PhoneNumber` and it checks against all the unassigned number treatment rules. Basically, we can check if our regular expression is correct.
...
Default value for the parameter is *FullInformation*. We can guess we will be able to limit what information is available in report mode for the meeting.
...
The parameter accepts string value. From my checking, if you enter any random value to it, you won't get any error. PowerShell will simply ignore it, as you can see on the screenshot below:

![](../../img/20220222-140637-axzce1w72b.png "MsftInternalProcessingMode parameter being ignored by PowerShell")

### *Sync application instance*

*ApplicationId* parameter has been added for *Sync-CsOnlineApplicationInstance* cmdlet. Now we can use the cmdlet  not only with object ID, but also with application ID.
